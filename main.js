document.addEventListener("DOMContentLoaded", function() {
    var input = document.getElementById("busqueda_pedido");

    input.addEventListener("input", function() {
        var filterValue = input.value.toLowerCase();

        var rows = document.querySelectorAll("#tabla_container table tr");

        rows.forEach(function(row) {
            var numberCell = row.getElementsByTagName("td")[1]; 

            if (numberCell) {
                if (numberCell.textContent.toLowerCase().indexOf(filterValue) > -1) {
                    row.style.display = "";
                } else {
...
clientesInput.addEventListener('keypress', (event) => {
    if (event.key === 'Enter') {
        ...
        for (let i = 1; i <= numClientes; i++) {
            ...
            num.innerText = 'Número:';
            const numI = document.createElement('input');
            numI.setAttribute('type', 'text');
            numI.setAttribute('placeholder', 'Ingrese el código de edifico');
